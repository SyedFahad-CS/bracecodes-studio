      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
          {filteredProjects.map((project, index) => (
            <ScrollReveal key={`${filter}-${index}`} delay={index * 100}>
              <Link to={`/work/${project.slug}`} className="group cursor-pointer block">
                {/* Image Container */}
                <div className="relative aspect-[3/2] rounded-[2.5rem] overflow-hidden mb-8 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] transition-all duration-500 group-hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] group-hover:-translate-y-2">
                  <img
                    src={`${project.image}?w=1000&q=85`}
                    alt={project.title}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-3 py-1.5 text-[10px] font-bold text-slate-600 bg-gradient-to-b from-slate-50 to-slate-100 border border-slate-200 rounded-xl shadow-[inset_0_1px_2px_rgba(0,0,0,0.04)] uppercase tracking-widest">
                        {tag}
                      </span>
                    ))}
                  </div>
