...
import { projectBySlugQuery, allProjectsQuery } from '@/features/projects/api/queries';
import type { SanityProjectFull, SanityProject } from '@/features/projects/types';
    ...
    return (
        <div className="bg-slate-50 min-h-screen">
            <Helmet>
                ...
                <meta property="og:title" content={`${project.title} | Bracecodes Case Study`} />
                <meta property="og:description" content={project.description} />
                {project.mainImage && (
                    <meta property="og:image" content={getOptimizedImageUrl(project.mainImage, 1200)} />
                )}
            </Helmet>
            {/* Hero Section */}
                ...
                <div className="container mx-auto max-w-7xl">
                    <ScrollReveal delay={300}>
                        <div className="rounded-2xl md:rounded-3xl overflow-hidden shadow-xl shadow-indigo-900/5 relative aspect-video md:aspect-21/9">
                            {project.mainImage ? (
                                <img
                                    src={getOptimizedImageUrl(project.mainImage, 1600, 95)}
                                ...
                                {project.gallery.map((img, idx) => (
                                    <ScrollReveal key={idx} delay={600}>
                                        <div className={`rounded-2xl overflow-hidden shadow-md ${idx % 3 === 0 ? 'aspect-21/9' : 'aspect-video'}`}>
                                            <img
                                                src={getOptimizedImageUrl(img, 1200, 90)}
