      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-14">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-block px-3 py-1 rounded-full bg-white border border-slate-200 text-indigo-600 font-bold text-xs tracking-widest uppercase mb-3 shadow-sm"
          >
            Our Methodology
          </motion.div>
          <motion.h2
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          {steps.map((s, i) => (
            <div key={i} className={`relative mb-12 md:mb-16 last:mb-0 flex flex-col md:flex-row items-center ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
              ...
              >
                <div className={`hidden md:flex flex-col ${i % 2 === 0 ? 'items-end' : 'items-start'}`}>
                  <span className="text-6xl font-black text-slate-100 mb-[-20px] z-0 select-none block leading-none">{s.step}</span>
                </div>
                <div className="relative z-10">
                  <span className="md:hidden text-4xl font-black text-slate-100 absolute -top-6 -left-4 -z-10">{s.step}</span>
                  <h3 className="text-lg font-bold text-slate-900 mb-2">{s.title}</h3>
                  <p className="text-slate-500 text-sm leading-relaxed mb-4">
                    {s.desc}
                  </p>
                  <ul className={`inline-block text-left space-y-2 bg-white p-3.5 rounded-xl border border-slate-100 shadow-sm ${i % 2 === 0 ? 'md:items-end' : ''}`}>
              ...
              >
                <div className="relative w-full aspect-4/3 max-w-xs bg-white rounded-2xl border border-slate-100 shadow-2xl shadow-slate-200/50 flex items-center justify-center p-4 overflow-hidden group">
                  <div className={`absolute inset-0 bg-linear-to-br opacity-5 group-hover:opacity-30 transition-opacity duration-500 ${i === 0 ? 'from-blue-500 to-cyan-500' :
                  ...
                  <s.visualIcon strokeWidth={1} className="w-20 h-20 text-slate-200 group-hover:scale-110 group-hover:text-indigo-100 transition-all duration-700" />
